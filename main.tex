...
% \todo{compatibility issues}
	...
	The \verb|\Space| command may be used when no bar is needed, or to add extra space between symbols. Its optional argument changes the amount of space. Default is 1, where 1 is the normal separation between symbols. Negative values may also be used (see subsection \ref{space}).
...
\item \textsl{sloped} makes a beam of three or more notes follow a slope from the first note to the last instead of being horizontal.  Two-note beams are always oblique. \todo{sloped creates non vertical end of beams}
...
\Text[yshift=0]{\ttfamily \textbackslash Note}
\Text[yshift=-0.5]{\ttfamily \{1\}\{12/10/<9/7\}}
\Note{1}{12/10/<9/7}
\Space[1.5]
\Text[yshift=-1.25,xshift=0.5]{\ttfamily \textbackslash Note[sloped,tuplet,swap]}
\Text[yshift=-1.75,xshift=0.5]{\ttfamily\{8\}\{0/4,1/5,2/6\}}
\Note[sloped,tuplet,swap]{8}{0/4,1/5,2/6}
\Space[1.5]
\Text[yshift=0]{\ttfamily \textbackslash Note[head=x,stem=7]}
\Text[yshift=-0.5]{\ttfamily \{16\}\{5\}}
\Note[head=x,stem=7]{16}{5}
\Barline[t=|.]
\end{mousik}
\end{center}

Notes with different durations may also be joined in the same beam group. In this case the first argument is a list. \texttt{-} means an eighth note, \texttt{=} a sixteenth note and $\equiv$ a thirty-second note. A \texttt{|} may be added before or after a symbol to mark where that level begins or ends. If there is no neighbouring note at that level, the side of the \texttt{|} also chooses the direction. The forms \texttt{//} and \texttt{///} are equivalent to \texttt{=} and $\equiv$. Inside a beam group, \texttt{!} leaves a space of 0.75 cm, and an empty item leaves a note position.
	\begin{center}
		\begin{mousik}
			\Clef
			...
			\Barline[t=|.]
		\end{mousik}
	\end{center}\todo{the first one looks too short and the last one too long, not half half}
...
\subsubsection{\texttt{\textbackslash Rest}}\label{rest}
	...
	The \verb|\Back| command moves the horizontal cursor backwards. Its optional argument is the number of note positions to step backwards, with 1 as default. It is useful for superimposing independent voices, as well as a manual alternative to chords.
	...
	\begin{center}
		\begin{mousik}
			...
			\TTT[-0.25]{Note\{8\}\{5,4\}}
			\TTT[-0.75]{Back[2]}
			\TTT[-1.25]{Rest[yshift=-0.625]\{8\}} 
			\TTT[-1.75]{Back[0]}
			\TTT[-2.25]{Note[swap]\{8\}\{2\}}
			\Note{8}{5,4}
			\Back[2]
			\Rest[yshift=-0.625]{8} 
			\Back[0] 
			...
			\Barline[t=|.]
		\end{mousik}
	\end{center}\todo{why 0.625?}\todo{why is back[0] needed here?}\todo{Looks like space[-2] works better than doing back? perhaps its better to have a separate section on using negative space for rests and polyphony}
	...
	When there are several notes, the \verb|\Accs| command might be useful. It accepts as an argument a list of pairs: type/height. Where there is no accidental, the spot can be left empty. It also accepts the \textsl{xshift} and \textsl{yshift} options.
...
\end{center}
	...
	The \verb|\MixedKey| command prints a change of key signature. It accepts two arguments: the old key signature and the new one. For example, \verb|\MixedKey{5}{2}| changes from five sharps to two sharps, and \verb|\MixedKey{2}{-3}| changes from two sharps to three flats.	
...
\end{lstlisting}
	...
	\todo{change name of mdot? to another reasonable thing that is not dot (as this exists already i think)}
	...
	When there are several notes, the \verb|\MDots| command can be useful. It accepts as an argument a list of pairs: symbols/height, where symbols can be $*$ or $**$. Where there are no dots, the spot can be left empty. It also accepts the \textsl{xshift} and \textsl{yshift} options.
	...
	\begin{center}
		\begin{mousik}		
			...
			\Barline[t=|.]
		\end{mousik}
	\end{center} \todo{this one is currently ok in terms of = spacing, but if i change it at one point i will need to check}
	...
	The \verb|\Slur| command draws a slur between two consecutive notes. It accepts two compulsory arguments: the absolute heights of its beginning and end. It accepts the same options as \verb|\Tie|: \textsl{angle}, \textsl{b}, \textsl{e}, \textsl{swap}, \textsl{width}, \textsl{xshift} and \textsl{yshift}.
	...
	The \verb|Slurred| environment draws a slur between the notes placed inside. It has two mandatory arguments: the heights of both beginning and end notes. It accepts the same options as \verb|\Slur|: \textsl{angle}, \textsl{b}, \textsl{e}, \textsl{swap}, \textsl{width}, \textsl{xshift} and \textsl{yshift}. \todo{angle in and out?}
...
\todo{these examples are not so good as they are too many but the line break should still exist}
	\begin{center}
		\begin{mousik}
			\Clef

			\Text{\ttfamily Crescendo[height=-3]}
			\begin{Crescendo}[height=-3]
				\foreach\i in {2,...,-1}{\Note{4}{\i}}
			\end{Crescendo}			

			\Text{\ttfamily Decrescendo}
			\begin{Decrescendo}
	...
	\begin{center}
		...
		\begin{mousik}
			\Clef
			\Text{\ttfamily begin=0.24,end=0.4}
...
\begin{mousik}
	...
	{\TTT{Tempo[yshift=2.5]\{8\}\{120\}}}
	\Note{4}{2}
	\Space[3]
	\Tempo[yshift=2.4]{2}{60}
	{\TTT[-0.5]{Tempo[yshift=2.4]\{2\}\{60\}}}
	\Note{4}{4}
	\Barline[t=|.]
\end{mousik}
\end{center}
\todo{for some reason the 2 needs 2.4 instead of 2.5}
	...
	The environment has a maximum width, which may be changed with the \textsl{long} option. When the cursor reaches that width, the next element triggers an automatic overflow. This starts a new staff line and repeats the current clef and key signature.
