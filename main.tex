...
It has some options:
	\begin{enumerate}[-]
		...
		\item \textsl{hsep} changes the base horizontal separation. Default is 8. Beams and parallel dot lists follow the same separation. It does not change \textsl{indent}.
		\item \textsl{accsep} changes the horizontal space for accidentals, represented by \texttt{!}, in units of \textsl{hsep}. Default is 0.5.
...
When there are several notes, the \verb|\Artics| command may be useful. It accepts as an argument a list of pairs: symbols/height. Where there are no symbols, the spot can be left empty. It also accepts the \textsl{xshift} and \textsl{yshift} options.
